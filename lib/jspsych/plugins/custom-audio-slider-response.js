var jsPsychCustomAudioSliderResponse = (function (jspsych) {
  'use strict';

  const info = {
    name: "custom-audio-slider-response",
    parameters: {
  ...
  class AudioSliderResponsePlugin {
    ...
    trial(display_element, trial, on_load) {
      ...
      const setupTrial = (labels_mul_idx = 0) => {
        ...
        // handling labels
        for (var j = 0; j < trial.labels_mul[labels_mul_idx].length; j++) {
          ...
          html +=
            ...
            label_width_perc +
            '%;">';
          html += '<span style="text-align: center; font-size: 80%;">' + trial.labels_mul[labels_mul_idx][j] + "</span>";
          html += "</div>";
        }
